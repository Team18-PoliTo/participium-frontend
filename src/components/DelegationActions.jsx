...
function DelegationActions({ report, onSuccess, onCancel }) {
    ...
    return (
        <div className="mt-3">
            <div className="report-desc-label" style={{ fontSize: "1.1rem", marginBottom: "1rem" }}>
                Delegate Report
            </div>
            ...
            <Dropdown className="report-desc-category-dropdown mb-4">
                <Dropdown.Toggle id="delegation-company-dropdown" disabled={submitting} className="w-100 d-flex justify-content-between align-items-center">
                    <span className="fw-medium">
                        {selectedCompanyId
                            ? companies.find(c => c.id === Number.parseInt(selectedCompanyId))?.name || "Company Selected"
                            : "Choose a company"}
                    </span>
                </Dropdown.Toggle>
